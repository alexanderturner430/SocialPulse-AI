/**
 * TensorFlow.js Threads Analysis - Direct Input, No API Calls
 */
...
async function predictEngagement(features) {
  const { textLen = 0, emojiCount = 0, likeCount = 0, replyCount = 0 } = features;
  const input = tf.tensor2d([[textLen / 1000, emojiCount / 10, likeCount / 10000, replyCount / 1000]]);
  ...
  const predictedLikes = Math.round(score * 10000 * (1 + emojiCount * 0.1) * Math.min(2, Math.max(0.1, textLen / 100)));
  const predictedReplies = Math.round(score * 1000 * Math.min(2, Math.max(0.1, replyCount / 100)));
  return { engagementScore: +score.toFixed(4), predictedLikes, predictedReplies, features };
}

async function classifyTopics(texts) {
  const arr = Array.isArray(texts) ? texts : [texts];
  const topicKeywords = {
    technology: ["tech","code","software","app","ai","data","web","digital","computer","programming","hack","dev","api","cloud","cyber"],
    politics: ["election","vote","president","government","policy","law","congress","democrat","republican","campaign","political"],
    sports: ["game","team","player","score","win","match","championship","nba","nfl","soccer","football","basketball"],
    entertainment: ["movie","music","show","celebrity","actor","series","netflix","concert","album","entertainment"],
    health: ["health","medical","doctor","cure","disease","fitness","wellness","mental","exercise","nutrition"],
    business: ["business","market","stock","company","startup","revenue","profit","invest","finance","economy"]
  };
...
async function clusterContent(texts) {
  ...
  const featuresWithCoord = features.map((f, i) => [...f.slice(0, 2), i]);
  const kmeans = [];
  for (let c = 0; c < numClusters; c++) kmeans.push([...featuresWithCoord[c]]);
