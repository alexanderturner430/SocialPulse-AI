/**
 * TensorFlow.js Mastodon Analysis - Direct Input, No API Calls
 */
...
async function analyzeStatusSentiment(texts) {
...
async function predictEngagement(features) {
  const { textLen = 0, boostCount = 0, favouriteCount = 0, replyCount = 0 } = features;
  const input = tf.tensor2d([[textLen / 1000, boostCount / 5000, favouriteCount / 10000, replyCount / 1000]]);
  ...
  const predictedBoosts = Math.round(score * 5000 * Math.min(3, Math.max(0.1, boostCount / 200)));
  const predictedFavourites = Math.round(score * 10000 * Math.min(3, Math.max(0.1, favouriteCount / 500)));
  const predictedReplies = Math.round(score * 1000 * Math.min(2, Math.max(0.1, replyCount / 50)));
  return { engagementScore: +score.toFixed(4), predictedBoosts, predictedFavourites, predictedReplies, features };
}

async function classifyTopics(texts) {
  const arr = Array.isArray(texts) ? texts : [texts];
  const topicKeywords = {
    technology: ["tech","code","software","app","ai","data","web","digital","programming","hack","api","cloud","cyber","open source","mastodon","fediverse"],
    politics: ["election","vote","president","government","policy","law","congress","campaign","political","rights","activism"],
    sports: ["game","team","player","score","win","match","championship","nba","nfl","soccer","football"],
    entertainment: ["movie","music","show","celebrity","actor","series","concert","album","streaming","anime","gaming"],
    health: ["health","medical","doctor","cure","disease","fitness","wellness","mental","exercise","covid"],
    social: ["friend","community","follow","post","share","reply","boost","favourite","thread","discuss"]
  };
...
async function compareStatuses(text1, text2) {
  ...
  return { status1: { preview: text1.substring(0, 100), wordCount: tokens1.length, sentiment: sent1.sentiment, score: sent1.score }, status2: { preview: text2.substring(0, 100), wordCount: tokens2.length, sentiment: sent2.sentiment, score: sent2.score }, similarity: { jaccard: +jaccard.toFixed(4), cosine: +(1 - cosSim).toFixed(4), sharedWords: intersection.length }, sharedKeywords: intersection.slice(0, 10) };
}
...
module.exports = { analyzeStatusSentiment, detectToxicity, predictEngagement, classifyTopics, extractKeywords, detectTrends, forecastGrowth, detectAnomalies, compareStatuses, clusterContent };
