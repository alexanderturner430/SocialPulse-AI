...
async function detectAnomalies(dataPoints, thresholdPercentile = 95) {
  if (!dataPoints || dataPoints.length === 0) return { error: "dataPoints cannot be empty" };
  ...
  const normalized = tensorData.sub(min).div(tf.scalar(range));
  const inputDim = 1;
  const encodingDim = 4;
  ...
  const errTensor = normalized.sub(predictions).square().mean(1);
  const errors = errTensor.dataSync();
  errTensor.dispose();
  predictions.dispose();
  normalized.dispose();
  tensorData.dispose();
...
async function clusterData(dataPoints, numClusters = 3) {
  ...
  if (data.length === 0) return { error: "dataPoints cannot be empty" };
  if (data[0].length === 0) return { error: "data points must have at least one dimension" };
  const tensor = tf.tensor2d(data);
  let centroids = tf.randomNormal([numClusters, data[0].length]);
  let assignments = tf.zeros([data.length], "int32");
  for (let iter = 0; iter < 20; iter++) {
    const dists = [];
    for (let c = 0; c < numClusters; c++) {
      const centroid = centroids.slice([c, 0], [1, -1]);
      dists.push(tensor.sub(centroid).square().sum(1));
      centroid.dispose();
    }
    const distTensor = tf.stack(dists, 1);
    const newAssignments = distTensor.argMin(1);
    dists.forEach(d => d.dispose());
    distTensor.dispose();
    assignments.dispose();
    assignments = newAssignments;
    const newCentroids = [];
    for (let c = 0; c < numClusters; c++) {
      const mask = assignments.equal(tf.scalar(c, "int32")).toFloat().expandDims(1);
      const count = mask.sum().add(tf.scalar(1e-8));
      newCentroids.push(tensor.mul(mask).sum(0).div(count));
    }
    const nextCentroids = tf.stack(newCentroids, 0);
    newCentroids.forEach(c => c.dispose());
    centroids.dispose();
