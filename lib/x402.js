...
function fallbackConfig() {
  ...
  return {
    ...
    priceAtomic: String(Math.round(DEFAULT_PRICE_USD * 10 ** USDC_DECIMALS)),
    network,
  };
}
...
function init(toolDefinitions) {
  ...
  try {
    ...
    if (!/PAY_TO_ADDRESS/.test(err.message)) throw err;
    config = fallbackConfig();
    _paymentsConfigured = false;
    _gateEnabled = false;
    console.error("x402 payments disabled: PAY_TO_ADDRESS not set; payment gate is off and tools run ungated.");
  }
  ...
  if (_paymentsConfigured) {
    state.facilitatorClient = buildFacilitatorClient(config);
    state.resourceServer = buildResourceServer(state.facilitatorClient, config);
    state.routes = buildRouteConfig(config, toolDefinitions);
    state.paymentMiddleware = paymentMiddleware(state.routes, state.resourceServer);
  }
...
function setGateEnabled(enabled) {
  if (enabled && !_paymentsConfigured) {
    throw new Error("PAY_TO_ADDRESS is not configured; cannot enable payment gate");
  }
