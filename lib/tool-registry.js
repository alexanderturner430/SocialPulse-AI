...
const toolRegistry = {
  ...
  "regression": async (args) => {
    ...
    if (!Array.isArray(data) || data.length === 0) {
      return { error: "dataPoints must be a non-empty array of { x, y } pairs or numeric values" };
    }
  ...
  "predict-twitter-engagement": async (args) => await tfTwitter.predictTweetEngagement(args.features || args.dataPoints || []),
  ...
  // === TF.js FACEBOOK ===
  "analyze-facebook-content": async (args) => await tfFacebook.analyzePostContent({ message: args.message, imageUrl: args.imageUrl }),
