...
async function videoComments(videoId, limit = 50) {
  const data = await callApi("commentThreads", {
    part: "snippet",
    videoId,
    textFormat: "plainText",
    maxResults: Math.min(Math.max(Number(limit) || 50, 1), 100)
  });
  return (data.items ?? []).map((item) => {
    const snippet = item.snippet?.topLevelComment?.snippet ?? {};
    return {
      id: item.id ?? null,
      text: snippet.textDisplay ?? null,
      author: snippet.authorDisplayName ?? null,
      likeCount: Number(snippet.likeCount) || null,
      publishedAt: snippet.publishedAt ?? null
    };
  });
}

module.exports = { isEnabled, searchVideos, videoDetails, videoComments };
